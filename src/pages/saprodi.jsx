...
import { kategoriSaprodi } from "../data/kategoriSaprodi";
import KategoryCard from "../components/fragments/KategoryCard";
import { allItemSaprodi, rekomendasiProdukSaprodi } from "../data/allItemSaprodi";
...
const Saprodi = () => {
  ...
  // Filter data allItemSaprodi berdasarkan input pencarian
  const filteredItems = allItemSaprodi.filter(
    (item) =>
      item.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.category.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // Filter data allItemSaprodi untuk mendapatkan item yang ada di rekomendasiProdukSaprodi
  const filteredrekomendasiProdukSaprodi = allItemSaprodi.filter((item) =>
    rekomendasiProdukSaprodi.includes(item.id)
  );
          ...
          {searchQuery === "" ? (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 mt-2 justify-items-center">
                {filteredrekomendasiProdukSaprodi.map((item) => (
                  <ProductCard
                    key={item.id}
                    id={item.id}
                    image={item.image}
